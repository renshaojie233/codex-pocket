...
const codexStreamPath = process.env.CODEX_STREAM_APK_PATH ||
  resolve(moduleDir, "..", "..", "outputs", "codex-stream-1.3.5.apk");
  ...
  req.on("end", () => {
    try {
      ...
      const diagnostic = {
        ...
        renderMode: String(parsed.renderMode || "").slice(0, 24),
...
const server = http.createServer((req, res) => {
  ...
  if ((req.method === "GET" || req.method === "HEAD") && url.pathname === "/download/codex-stream.apk") {
    serveApk(req, res, {
      ...
      fileName: "codex-stream-1.3.5.apk",
      etagName: "codex-stream-1.3.5",
    });
    return;
  }
