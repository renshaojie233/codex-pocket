...
- 后台持续预同步最近任务，打开即显示缓存；向上滚动时按批加载更早历史。
- 实时同步回复、思考状态、工具进度，以及运行中的引导消息；回复在手机端平滑逐字展开。
- 运行时实时展开进度、思考、命令与输出；最终回复出现后自动合并为一条可再次展开的折叠栏。
- 折叠栏显示浅色实时进度；Wi-Fi / 流量切换后主动重建链路并持续快速恢复。
- 支持 Markdown、LaTeX 公式、手机图片上传、视频、全屏缩放和媒体保存。
- 将 Codex 生成的 PPTX、DOCX、PDF、表格、压缩包和代码文件安全下载到手机并调用对应应用打开。
...
串流画面右下角的键盘图标会展开可收起的特殊键面板，提供方向键、Esc、Tab、
Enter、Ctrl+C 和 Ctrl+L；长按键盘图标可直接打开手机输入法。
